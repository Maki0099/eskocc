...
const WeeklyLeaderboard = () => {
  ...
  useEffect(() => {
    let active = true;
    const load = async () => {
      const { data } = await supabase.rpc("get_weekly_leaderboard" as any);
      if (!active) return;
      setRows((data as any as WeeklyRow[]) || []);
      setLoading(false);
    };
    load();
    return () => {
      ...
      <Card className="animate-fade-up">
        ...
        <CardContent>
          <p className="text-center text-muted-foreground py-4">
            Tento týden zatím nikdo z propojených členů nejel. Kdo bude první?
          </p>
        </CardContent>
      </Card>
    );
